...
namespace
{
    constexpr qsizetype kMd5HexLength = 32;
...
int main()
{
    ...
    if (!check(confirmedNeither.upper.styleId == QStringLiteral("NEW-UPPER") && confirmedNeither.lower.styleId == QStringLiteral("NEW-LOWER"),
    ...
    if (!check(loaded && loaded->photoFileName == QStringLiteral("photo.jpg") && loaded->photoFileSize == QFileInfo(imagePath).size() &&
                   loaded->photoMd5.size() == kMd5HexLength && loaded->upper.styleId == QStringLiteral("TOP001") && !loaded->upper.confirmed &&
